...
const BookingModal = ({ worker, onClose, onBookingComplete }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast("Booking request sent successfully! Check your dashboard for updates.");
      onBookingComplete();
    } catch (err) {
      addToast(err.response?.data?.message || "Failed to send booking request.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="modal" style={{ display: "flex", zIndex: 1000 }}>
      <div className="modal-content" style={{ maxWidth: "500px", padding: "30px", borderRadius: "16px" }}>
        <span className="close" onClick={onClose}>&times;</span>
        <h2 style={{ color: "var(--primary)", marginBottom: "10px" }}>Book Service</h2>
        <p style={{ marginBottom: "20px", color: "var(--text-light)" }}>
          Requesting <strong>{worker.workerDetails?.skill}</strong> from <strong>{worker.fullName}</strong>
        </p>
        ...
        <form onSubmit={handleSubmit} className="register-full-form">
          <div className="register-grid-2">
            <div className="register-input-group">
              ...
              <input type="date" name="date" required value={formData.date} onChange={handleChange} min={today} />
            </div>
          ...
          <div className="register-input-group">
            ...
            <textarea 
              ...
              style={{ width: "100%", padding: "10px", borderRadius: "8px", border: "2px solid rgba(96, 108, 56, 0.2)", minHeight: "80px" }}
